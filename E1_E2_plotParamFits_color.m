function [curveParams]=E1_E2_plotParamFits_color(E2_params,E1_params,E1Rev_params)
%% Fit exp learning curves and plot the parameter fits for E1 and E2
% 4.15.2015-Created
% 11.25.2015-E1_E2_plotParamFits_color-Color version
...
for ip=2:4
    % Dummy for legend
    if ip==2
        a=errorbar((-24:-5)-1000,E1_paramsBlock_mean(:,ip),E1_paramsBlock_std(:,ip),'k','LineWidth',4,'LineStyle',lineStyle{ip-1},'Color',currCol(1,:),'Marker',markerStyle{ip-1},'MarkerSize',14,'LineWidth',3);
    elseif ip==3
        a=errorbar((-24:-5)-1000,E1_paramsBlock_mean(:,ip),E1_paramsBlock_std(:,ip),'k','LineWidth',4,'LineStyle',lineStyle{ip-1},'Color',currCol(2,:),'Marker',markerStyle{ip-1},'MarkerSize',15,'LineWidth',3);
    elseif ip==4
        a=errorbar((-24:-5)-1000,E1_paramsBlock_mean(:,ip),E1_paramsBlock_std(:,ip),'k','LineWidth',4,'LineStyle',lineStyle{ip-1},'Color',currCol(3,:),'Marker',markerStyle{ip-1},'MarkerSize',4,'LineWidth',3,'MarkerFaceColor',[0 0 0]);
    end
    handles(ip-1)=a;
    
    % Actual
    errorbar([1 2 8],E2_paramsBlock_mean(:,ip),E2_paramsBlock_std(:,ip),'k','LineWidth',4,'LineStyle',lineStyle{ip-1},'Color',currCol(ip-1,:));
    if ip==2
        eh=errorbar((-24:-5),E1_paramsBlock_mean(:,ip),E1_paramsBlock_std(:,ip),'k.','Color',currCol(1,:)-.2,'Marker',markerStyle{ip-1},'MarkerSize',12,'LineWidth',3);
    elseif ip==3
        eh=errorbar((-24:-5),E1_paramsBlock_mean(:,ip),E1_paramsBlock_std(:,ip),'k.','Color',currCol(2,:)-.2,'Marker',markerStyle{ip-1},'MarkerSize',12,'LineWidth',3);
    elseif ip==4
        eh=errorbar((-24:-5),E1_paramsBlock_mean(:,ip),E1_paramsBlock_std(:,ip),'k.','Color',currCol(3,:)-.2,'Marker',markerStyle{ip-1},'MarkerSize',4,'LineWidth',3,'MarkerFaceColor',[.5 .5 .5]);
    end
    errorbar_tick(eh,inf);
    noiseParams=mean(curveParams{ip},1);
    plot(-24:-5,noiseParams(2)+((noiseParams(1)-noiseParams(2))*exp(-(0:19)/noiseParams(3))),'k','LineStyle',lineStyle{ip-1},'Color',currCol(ip-1,:))
    errorbar(-3:-1,E1Rev_paramsBlock_mean(:,ip),E1Rev_paramsBlock_std(:,ip),'k','LineStyle',lineStyle{ip-1},'Color',currCol(ip-1,:));
end
