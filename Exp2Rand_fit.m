function [allParam, allLlk,allMle]=Exp2Rand_fit(data,center,envrad)
% Find the best parameter fits
% 4.8.2015-Created
% 6.16.2015-Exp2Rand_fit-Introducing random guessing noise parameter
...
allParam=nan(3,numSamps,5); % sessions x samples x parameters 
allLlk=nan(3,numSamps);
allMle=nan(10,11,size(allResp,1),3);
init=[60 1/3 1/3 1/3 100]; % Initial parameters. Set probability to 0 to eliminate mixture weight
probPrior=[1 1 1];
for is=1:3
    disp(is)
    ...
    if ~exist(fullName)
        ...
        [params, llk,Mle]=Exp2Rand_gibbs(allRespSess,allTargSess,center,numBurn,numSamps,init,probPrior,envrad);
