function [llk_responses, data]=Exp1Rand_ResponseLikelihood(allResp, allTarg,allRep,allPrev,center,currParams,envrad)
...
for is=1:numSubj
    if ~(isnan(allResp(is,:,1))>0)
        ...
        % Rand-Truncated sd
        rand_llk=mvnpdf([xr' yr'], [centerX(1) centerY(1)], (eye(2)*currParams(6)^2)/(1-exp(-envrad^2/(2*currParams(6)^2))));
        ...
        %% And sample
        [targ, miss, r_err,r_err2, curr_dist,curr_distRand]=Exp2Rand_samps(llk_curr,xr,yr,xt,yt,center);
